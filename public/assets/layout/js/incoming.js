...
class RequestComponent extends React.Component {
    ...
    render() {
        ...
        return (            
            ...
            <div><h5>Drop Location</h5><p>{this.props.request.request.d_address}</p></div> :'' }
            {this.props.request.service.admin_service == "DELIVERY" ?
            <div><h5>Dropping Point{this.props.request.request.deliveries.length && this.props.request.request.deliveries.length > 1 ? 's ( ' + (this.props.request.request.deliveries.length) + ' )' : ''}  </h5> <p>{this.props.request.request.deliveries.map((delivery) => (<p style={{ marginBottom: 0, paddingTop: 0 }} key={delivery.id}> {delivery.d_address} </p>))}</p></div> : ''}
