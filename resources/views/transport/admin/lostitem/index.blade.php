...
@section('title') {{ __('transport.admin.lostitem.title') }}@stop
...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            <span class="text-uppercase page-subtitle">{{ __('transport.admin.lostitem.title') }}</span>
            <h3 class="page-title">{{ __('transport.admin.lostitem.title') }}</h3>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    <h6 class="m-0 pull-left">{{ __('transport.admin.lostitem.title') }}</h6>
                    @permission('lost-item-create')
                    <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i> {{ __('transport.admin.lostitem.add') }}</a>
                    @endpermission
                </div>
                ...
                <thead>
                    <tr>
                            ...
                            <th data-value="ride_request_id">{{ __('transport.admin.lostitem.lost_id') }}</th>
                            <th data-value="user_id">{{ __('transport.admin.lostitem.lost_user')  }}</th>
                            <th data-value="lost_item_name">{{ __('transport.admin.lostitem.lost_item')  }}</th>
                            <th data-value="comments">{{ __('transport.admin.lostitem.lost_comments')  }}</th>
...
$(document).ready(function() {
    ...
    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/lostitem/create') }}", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/lostitem/') }}/"+$(this).data('id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "columns": [
            ...
            { "data": "ride_request_id", render: function (data, type, row) {
                return row.riderequests?row.riderequests.booking_id:'';
               }
            },
            ...
            { "data": "id", render: function (data, type, row) {
                var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                   '<ul class="dropdown-menu">';
                    if({{Helper::getDemomode()}} != 1){
                        button +='@permission("lost-item-edit")<li><a href="javascript:;" data-id="'+data+'" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>@endpermission<li>@permission("lost-item-delete")<a href="javascript:;" data-id="'+data+'" class="dropdown-item delete"><i class="fa fa-trash"></i>&nbsp;Delete</a> @endpermission</li>';
                    }
                    button +='</ul> </li></div>';
                 return  button;
               }
