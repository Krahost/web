...
$(document).ready(function() {
    ...
    $('.add').on('click', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/transporttype/create') }}", function(data) {
    ...
    $('body').on('click', '.edit', function(e) {
        e.preventDefault();
        $.get("{{ url('admin/transporttype/') }}/"+$(this).data('id')+"/edit", function(data) {
            $('.crud-modal .modal-container').html("");
            $('.crud-modal .modal-container').html(data);
        });
        $('.crud-modal').modal('show');
    ...
    table = table.DataTable( {
        ...
        "columns": [
            ...
            { "data": "status" ,render: function (data, type, row) {
                ...
                if(data ==1){
                    return "@permission('transport-type-status')<label class='switch'><input type='checkbox' class='status_enable switch-warning' checked data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"

                    }else{
                    return "@permission('transport-type-status')<label class='switch'><input type='checkbox' class='status_enable'  data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"
                    }
            }
            },
            { "data": function (data, type, row) {
                ...
                if(data.status ==1){
                    ...
                    var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                           ...
                           button +='@permission("edit-vehicle-type")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>@endpermission';
                            button +='</ul> </li></div>';
                         return  button;
