        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    ...
                    @permission('add-delivery-vehicle-type')
...
$(document).ready(function() {
    ...
    table = table.DataTable( {
        ...
        "columns": [
            ...
            { "data": "delivery_type_id", render: function (data, type, row) {
                return row.delivery_type ? row.delivery_type.delivery_name : '';
                }
             },
            { "data": "weight" },
            { "data": "length" },
            ...
            { "data": function (data, type, row) {
                ...
                if({{Helper::getDemomode()}} != 1){
                    ...
                    <li>@permission('delivery-vehicle-type-price')<a href="javascript:;" data-toggle="modal" data-target="#service_type_2" onclick="priceCountries(`+data.id+`)" class="dropdown-item price"><i class="fa fa-money"></i> Price</a> @endpermission</li>
                    <li>@permission('delivery-vehicle-type-status')<a href="javascript:;" data-id="`+data.id+`" data-value="`+data.status+`" class="dropdown-item status"><i class="fa fa-plus"></i>&nbsp;Status</a> @endpermission</li> 
                    <li>@permission('edit-delivery-vehicle-type')<a href="javascript:;" data-id="`+data.id+`" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> @endpermission</li> 
                    </ul> </li></div>`;
                 return  button;
                }
                else{
        ...
        success: function(data) {
            ...
            $.each( data.responseData,function(key,country){
                ...
                $.each( country.city,function(key1,city){
                    ...
                    countryCityList +=  `<a href="#" class="list-group-item cityActiveClass  `+cityActiveClass+`" data-value="`+country.currency+`" onclick="getData(`+id+`,`+city.id+`,`+country.id+`)" id="`+city.id+`"><span>`+city.city_name+`</span></a>`;
                });
            });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data.responseData.price.length !=0){
                        ...
                        $('#weight_price').val(priceData.weight_price);
                        ...
                        $("#ride_price_id, #distance, #weight, #weight_price, #distance, #fixed, #price, #commission, #fleet_commission, #tax, #peak_commission").val('');
                    }
...
$('body').on('click', '#calculator', function(event) {   
    ...
    function priceInputs(cal){
        ...
        if(cal=='DISTANCE'){
            ...
            $("#weight_price").prop('disabled', true);
            $("#weight_price").prop('required', false);
            ...
            $("#weight_price").prop('disabled', false);
            $("#weight_price").prop('required', true);
            $("#changecal").text('BP + (TKgs - BKgs*PKgm)');
        }
        else if(cal=='DISTANCEWEIGHT'){
            $("#price,#distance,#weight","#weight_price").prop('disabled', false);
            $("#price,#distance,#weight","#weight_price").prop('required', true);
            ...
            $("#price,#distance").prop('disabled', false);
            $("#price,#distance").prop('required', true);
            $("#weight,#weight_price").prop('disabled', true);
            $("#weight,#weight_price").prop('required', false);
            $("#changecal").text('BP + (T{{__("transport.admin.vehicletype.distance")}}-BD*P{{__("transport.admin.vehicletype.distance")}})');
        }
    }
