                              ...
                              <div class="col-md-6 pl-0">
                                 <div id="ride-book" class="col-sm-12 col-md-12 p-0 form-section">
                                    ...
                                    <div class="col-md-12 col-sm-12 col-lg-12 p-0 mt-5 btn-section">
                                       <a id="ride-now" class="btn btn-primary ">@lang('delivery.user.ride_now') <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                                    </div>
                                 </div>
...
<script>
    ...
    var current_latitude = localStorage.current_latitude ? localStorage.current_latitude : 13.0574400;
    var current_longitude = localStorage.current_longitude ? localStorage.current_longitude : 80.2482605;
...
<script>
   ...
   var destination_content = `<div class="field-box col-md-12 col-sm-12 col-lg-12 p-0" style="background: #fff; box-shadow: 0px 5px 10px #ccc; border: 1px solid #1FBCC4; padding: 5px !important; border-radius: 15px; margin: 10px 0;">
                                             ...
                                             <div class="productListContent" style="display: none;">
                                                   ...
                                                   <div class="form-group">
                                                      <input type="text" class="form-control" name="receiver_instruction" placeholder="Instruction" id="receivarInstruction" autocomplete="off">
                                                   </div>
                                                   <div class="form-group">
                                                      <div class="row">
                                                         <div class="col-md-4">
      ...
      $(document).ready(function(){
        ...
        function usercard(payment){
         ...
         $("#book-now, #schedule-later").click(function() {
            ...
            var data = {};
            data["delivery_mode"] = $('select[name=delivery_mode]').val();
            ...
            data["payment_by"] = $('select[name=payment_by]').val();
            data["card_id"] = $('select[name=card_id]:selected').val();
            data["delivery_type_id"] = $('select[name=delivery_type_id]').val();
            data["receiver_name"] = $("input[name=receiver_name]").map(function() { return $(this).val(); }).get();
            data["receiver_mobile"] = $("input[name=receiver_mobile]").map(function() { return $(this).val(); }).get();
            data["d_latitude"] = $('input[name="d_latitude[]"]').map(function() { return $(this).val(); }).get();
            data["d_longitude"] = $('input[name="d_longitude[]"]').map(function() { return $(this).val(); }).get();
            data["d_address"] = $('input[name="d_address[]"]').map(function() { return $(this).val(); }).get();
            data["distance"] = $('input[name="distance[]"]').map(function() { return $(this).val(); }).get();
            data["package_type_id"] = $("select[name=package_type_id]").map(function() { return $(this).val(); }).get();
            data["receiver_instruction"] = $("input[name=receiver_instruction]").map(function() { return $(this).val(); }).get();
            data["weight"] = $("input[name=weight]").map(function() { return $(this).val(); }).get();
            data["is_fragile"] = $("input[name=is_fragile]").map(function() { return $(this).is(':checked') ? 1 : 0 ; }).get();
