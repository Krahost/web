   ...
   <div class="modal-dialog min-70vw">
      <div class="modal-content">
         ...
         <div class="modal-header">
            <h4 class="modal-title m-0">@lang('delivery.user.ride_details')</h4>
         ...
         <div class="modal-body ">
          <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-12 float-left">
               ...
               <div class="mytrip-right my_trips">
                  <div class="fare-break ">
                    <h5>@lang('delivery.user.Booking_ID') <span>
            ...
            <div id="delivery_trips" class="del-list-box">          
            </div>
      ...
      <div class="clearfix ">
         <div class="dis-center col-md-12 p-0 dis-center">
            <ul class="nav nav-tabs">
               <li class="nav-item">
                  <a class="nav-link active" data-toggle="tab" href="#my_trips">@lang('delivery.user.my_trips')</a>
               </li>
               <li class="nav-item">
                  <a class="nav-link" data-toggle="tab" href="#upcoming_trips">@lang('delivery.user.upcoming_trips')</a>
               </li>
            </ul>
         </div>
                     ...
                     <table  id="my_trips_grid" class="table  table-striped table-bordered w-100">
                        <thead>
                           <tr>
                               ...
                               <th>@lang('delivery.user.pickup_location')</th>
                               <th>@lang('delivery.user.drop_location')</th>
      ...
      $.ajax({
         ...
         success:function(data){
            ...
            $.each(result,function(key,item){
               ...
               let image = "https://maps.googleapis.com/maps/api/staticmap?autoscale=1&size=620x380&maptype=terrian&format=png&visual_refresh=true&markers=icon:" + window.url + "/assets/layout/images/common/marker-start.png%7C" + result.s_latitude + "," + result.s_longitude;

              for (var i in result.deliveries) {
                  image += "&markers=icon:" + window.url + "/assets/layout/images/common/marker-end.png%7C" + result.deliveries[i].d_latitude + "," + result.deliveries[i].d_longitude;
              }

               image += "&path=color:0x191919|weight:3|enc:" + result.route_key + "&key=" + getSiteSettings().site.browser_key;



               $('.map_key_img').attr('src', image);
               ...
               var totalAmount = 0;
               for(var i in result.deliveries) {
                  deliveryLocations += result.deliveries[i].d_address+'<br />';
                  if(result.deliveries[i].payment !=null)
                    totalAmount += result.deliveries[i].payment.total;
                  else
                    totalAmount +=0;
               }
               $('.to_address').html(deliveryLocations);
               ...
               $('#delivery_trips').html('');
                $.each(result.deliveries,function(key,value){
                  ...
                  if(payment != null){
                    delivery_details += '<div class="delivery_trips_detail"><div class="fare-break "> ';     
         ...
         { "data": "id" ,render: function (data, type, row, meta) {
            ...
            { "data": "d_address" ,render: function (data, type, row) {
               ...
               if(row.deliveries != null) {
                  for(i in row.deliveries) {
                     drop  += row.deliveries[i].d_address+ `<br />`;
                  }
            ...
            { "data": "deliveries" ,render: function (data, type, row) {
               var total_amount  = 0;
               if(row.deliveries != null) {
                  for(i in row.deliveries) {
                     total_amount  += row.deliveries[i].payment ? row.deliveries[i].payment.total : '';
                  }

               }
               return row.user.currency_symbol+ total_amount;
               }
            },
            { "data": "provider_id" ,render: function (data, type, row) {
              if(row.payment !=null)
              {
                if(row.payment.is_partial != 1 && row.payment.wallet > 0)
                {
                    return 'WALLET';
                }
                else{
                    return row.payment.payment_mode!= null ? row.payment.payment_mode:'NA';
                }
              }else{
                return 'NA';
              }
            ...
            { "width": "124px","data": "id", render: function (data, type, row) {
               ...
               if(row.user_rated == 0 && row.status == 'COMPLETED') {
                     html += `<a href="{{ url('/delivery') }}/`+row.delivery_type_id+`/delivery?id=`+row.id+`"><i class="fa fa-eye view-icon tripdetails" data-id = `+data+` data-toggle="tooltip" title="View"></i></a>`;
               } else {
