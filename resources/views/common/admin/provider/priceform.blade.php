...
<div class="row">
    ...
    <div class="border-rightme myprice">

    </div>
    </div>
    <div class="col-md-8">
    <div class="box-card price_lists_sty priceBody">
...
function getData(service){
        ...
        var html = `<thead>
            ...
            for (var i in result) {
                if(result[i].vehicle){
                    ...
                    btn=`<button class="btn btn-block btn-block btn-success view_vechile" data-value=`+service+` data-type=`+result[i].vehicle.type+`  data-id =`+result[i].vehicle.id+`>View Vehicle</button>`;
                    html += `<tbody>
                                <tr>
                                <td>`+j+`</td>
                    ...
                    if(result[i].vehicle.status == "ASSESSING" || result[i].status =="ASSESSING")
                        $('.approve_all').show();
                    else
                        $('.approve_all').hide();
                }
                ...
                if(result[i].provideradminservice){
                    ...
                    html += `<tbody>
                                <tr>
                                 ...
                                 <td>`+result[i].provideradminservice.status+`</td>
                                 <td>`+result[i].provideradminservice.base_fare+`</td>

                                 </tr>
                                 </tbody>`;
                if(result[i].provideradminservice.status == "ASSESSING" )
                    $('.approve_all').show();
                }
       ...
       var html = `<input type="hidden" class="service_names" value=`+service+`>
                     <thead>
                        <tr>
                           <th>Sno</th>
                           <th>Document Name</th>`;
                           if(service=='SERVICE') html+=`<th>Category</th>`;
                           html+=` <th>Status</th>
                           <th>@lang('admin.action')</th>
                        </tr>
                    </thead>`;
    ...
    if(totaldocuments[service] != undefined){
            ...
            if(result[i].document){
                ...
                html += `<tbody>
                            <tr>
                                ...
                                if(service=='SERVICE')
                                {
                                    if(result[i].document.service_category !=null)
                                    {
                                        html +=`<td>`+result[i].document.service_category.service_category_name +`</td>`;
                                    }else{
                                         html +=`<td> </td>`;
                                    }
                                ...
                                html +=`<td><span class="`+service+`" id='state`+result[i].id+`'>`+result[i].status+`</span></td>
                                <td>`+btn+`</td>
                             </tr>
                            </tbody>`;
        ...
        html += `<tbody>
                    <tr>
                    <td colspan='5'>No Document Uploaded</td>
                    </tr>
                </tbody>`;
   }
 ...
 $(document).on('click', '.view_vechile', function(){
  ...
  if(model_name=="ORDER"){
    ...
    $('#vechile_id').val(vechile_details[vechile_id].id);

    $('#vechile_type').val($(this).data('type'));
    $('#service_type').val($(this).data('value'));
    ...
    $('.img1').attr('src', vechile_details[vechile_id].picture1);
        ...
        if(vechile_details[vechile_id].status =='ASSESSING'){
            $("#approve_vechile").html(`<button class="btn btn-success btn-large  vehicles_approve pull-left"  style = "margin-right:20px;">Approve</button>`);

            $("#delete_vechile").html(`<button data-id=`+vechile_details[vechile_id].id+` data-value='' class='btn btn-danger btn-large doc-delete pull-left vehiclesreject_reason' style = "margin-right:20px;">Rejected</button>`);
...
$(document).on('click', '.view_button', function(e){
    ...
    $('.cancel').on('click', function(){
     ...
     $(document).on('click', '.documentreject_reason', function(){
        $('#document_type').val($(this).data('type'));
        $('#document_service').val($(this).data('value'));
        $('#document_reason').val('');
        $(".documentlist").hide();
        $("#documents_reject").show();
     });

    $(document).on('click', '.vehiclesreject_reason', function(){
        $('#reason').val('');
        $("#vehicle_details").hide();
        $("#vehicles_reject").show();
     });
    ...
    $(document).on('click', '.document_approve', function(){
      ...
      $(document).on('click', '.vehicles_approve', function(){

        var id =$('#vechile_id').val();
        $.ajax({
            type:"GET",
            url: getBaseUrl() + "/admin/provider/approve_vehicle/"+id,
            headers: {
                Authorization: "Bearer " + getToken("admin")
            },
            success:function(data){
        ...
        $('body').on('click', '.vehicle_rejects', function() {
        ...
        data.append( 'vechile_id', vechile_id );
        data.append( 'vechile_type', vechile_type );
        data.append( 'service_name', service_name );

        $.ajax({

            url:getBaseUrl() + "/admin/provider/vehicle_reject",
            type:"POST",
            processData: false,
            contentType: false,
            secure: false,
            ...
            success:function(data){
                alertMessage("Success", 'Vehicle Reject Successfully', "success");
                providerservice={};
                totaldocuments={};
                services={};
                total_deatils();
    ...
    $(document).on('click', '.approve_all', function(){
        ...
        $.ajax({
            type:"GET",
            url: getBaseUrl() + "/admin/provider/approveall/"+id+"?type="+service,
            headers: {
                Authorization: "Bearer " + getToken("admin")
            },
            success:function(data){
  ...
  $('body').on('click', '.document_rejects', function() {

        var data = new FormData();
        var document_id = $('#document_id').val();
        var document_type = $('#document_type').val();
        var document_reason = $('#document_reason').val();

        var document_service= $('#document_service').val();
        ...
        data.append( 'document_reason', document_reason );
        data.append( 'provider_id', id );
        data.append( 'document_id', document_id );
        data.append( 'document_type', document_type );
        data.append( 'document_service', document_service );

        $.ajax({

            url:getBaseUrl() + "/admin/provider/document_reject",
            type:"POST",
            processData: false,
            contentType: false,
            secure: false,
            ...
            success:function(data){
                alertMessage("Success", 'Document Reject Successfully', "success");
                providerservice={};
                totaldocuments={};
                services={};
                total_deatils();
                getData(document_service);
                $(".documentdetails").modal('hide');
                $('.crud-modal').css({'overflow-y': 'auto',"overflow-x": "hidden"});
            }
        });
