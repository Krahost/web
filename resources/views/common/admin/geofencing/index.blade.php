...
<div class="main-content-container container-fluid px-4">
    <div class="page-header row no-gutters py-4">
        <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
            ...
            <h3 class="page-title">{{ __('Geofence List') }}</h3>
        </div>
    </div>
    ...
    table = table.DataTable( {
        ...
        "columns": [
            ...
            { "data": "status" ,render: function (data, type, row) {
                // console.log(data);
                if(data ==1){
                        return "@permission('geofence-status')<label class='switch'><input type='checkbox' class='status_enable switch-warning' checked data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"
                   }else{
                    return "@permission('geofence-status')<label class='switch'><input type='checkbox' class='status_enable'  data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"
                   }
                }
            },
            { "data": function (data, type, row) {
                ...
                if(data.status ==1){
                    ...
                    var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                           ...
                           button +='@permission("geofence-edit")<li><a href="{{ url('admin/geo-fencing') }}/'+data.id+'/edit" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>@endpermission';
                           button +='@permission("geofence-delete")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item delete"><i class="fa fa-trash"></i> Delete</a> </li>@endpermission';
                            button +='</ul> </li></div>';
                         return  button;
