...
$('body').on('click', '.serviceBlock', function(e) {
   ...
   var userSettings = getUserDetails();
      //For list category
   $.ajax({
      url:  getBaseUrl() + "/user/service_sub_category/"+id+"?city_id="+userSettings.city_id,
      type: "GET",
      beforeSend: function(request) {
        showLoader();
      },
            ...
            for(var i in result) {
                  ...
                  if(result[i].service && result[i].service.admin_service == 'ORDER') {
                     ...
                     $('#rides').append(`<div class="service-list col-md-3 col-lg-2 col-sm-12 p-0">
                        ...
                        <div class="service-content">
                              <h3 class="service-title">`+ result[i].title +`</h3>
                                 <a href="`+ url + `/` + result[i].menu_type_id +`?city_id=`+userSettings.city_id+`" class="link-arrow">Book Now<i class="icon ion-ios-arrow-right"></i></a>
                        </div>
                        </div>
                     </div>`);
                  }
