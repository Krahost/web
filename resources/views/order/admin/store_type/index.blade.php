        ...
        "columns": [
            ...
            { "data": "name" },
             { "data": "status", render: function (data, type, row) {
                if(data ==1){
                        return "@permission('shop-type-status')<label class='switch'><input type='checkbox' class='status_enable switch-warning' checked data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"
                   }else{
                    return "@permission('shop-type-status')<label class='switch'><input type='checkbox' class='status_enable'  data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"
                   }
            }},
            { "data": function (data, type, row) {
                ...
                var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                       '<ul class="dropdown-menu"> @permission("shop-type-price")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item priceform"><i class="fa fa-plus"></i> Price</a> </li>@endpermission';
                       button +='@permission("edit-shop-type")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>@endpermission';
                        button +='</ul> </li></div>';
                     return  button;
