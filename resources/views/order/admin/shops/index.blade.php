...
$(document).ready(function() {
    ...
    table = table.DataTable( {
        ...
        "columns": [
            ...
            { "data": "status" ,render: function (data, type, row) {
                ...
                if(data ==1){
                        return "@permission('shop-status')<label class='switch'><input type='checkbox' class='status_enable switch-warning' checked data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"
                   }else{
                    return "@permission('shop-status')<label class='switch'><input type='checkbox' class='status_enable'  data-id='"+row.id+"' data-value='"+row.status+"'> <span class='slider round'></span></label>@endpermission"
                   }
            }
            },
            ...
            { "data": function (data, type, row) {
                ...
                if({{Helper::getDemomode()}} != 1){
                    var button='<div class="input-group-btn action_group"> <li class="action_icon"> <button type="button"class="btn btn-info btn-block "data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true" title="View"></i></button>'+
                           '<ul class="dropdown-menu">@permission("shop-category")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item category"><i class="fa fa-plus"></i>Category</a>'+
                            '</li>@endpermission  @permission("shop-items")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item items"><i class="fa fa-plus"></i> Items</a> </li>@endpermission ';
                            if(data.storetype.category=='FOOD' && data.storetype!=null){
                             button +='@permission("shop-items")<li > <a class="dropdown-item addon" data-id="'+data.id+'"  href="javascript:;"><i class="fa fa-plus" aria-hidden="true" title="Add on"></i>AddOn</a> </li>@endpermission'
                            }

                             button +='@permission("edit-shop")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item edit"><i class="fa fa-edit"></i> Edit</a> </li>@endpermission @permission("shop-log")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item logs"><i class="fa fa-database"></i> Logs</a> </li>@endpermission @permission("shop-wallet-details")<li><a href="javascript:;" data-id="'+data.id+'" class="dropdown-item wallet"><i class="fa fa-google-wallet"></i> Wallet Details</a> </li>@endpermission';
                        button +='</ul> </li></div>';
                     return  button;
                }
    ...
    $('body').on('click', '.addon', function(e) {
       window.location.replace("/admin/shopsaddon/"+$(this).data('id')+"/index");

    });

    $('body').on('click', '.category', function(e) {
       window.location.replace("/admin/shopscategory/"+$(this).data('id')+"/index");

    });

    $('body').on('click', '.items', function(e) {
       window.location.replace("/admin/shopsitems/"+$(this).data('id')+"/index");
