                           ...
                           <table class="table">
                              <tbody>
                                 ...
                                 <tr class="my_wallet">
                                    ...
                                    <td class="right">
                                       <input type="checkbox" class="tuswalt" name="wallet" data-amount="0" value="1" id="wallet"   />
                                    </td>
                                 </tr>
                                 <tr>
                                    <td class="pl-0 b-0">
                                 ...
                                 <tr style="background: #FFF6F4; color: #BC6750; border: 1px solid #BC6750;">
                                    <td colspan="2"><label for="leave_at_door"><input id="leave_at_door" type="checkbox" name="leave_at_door" value="1" /> <b>Leave order at door or gate</b> <br>Opt for no-contact delivery & our delivery executive will leave it at your door/gate (not applicable on COD orders)</label></td>
                                 </tr>
                              </tbody>
                           </table>
                           </form>
...
function loadpromocode(){
   $("#applypromo").val('');
    var search = '?';
    if($('#store_id').val()){
      search += '&store_id='+$('#store_id').val();
    }
    if($('#total_amount').val()){
      search += '&total_amount='+$('#total_amount').val();
    }
   $.ajax({
   url: getBaseUrl() + "/user/store/promocodelist"+search,
   type:"GET",
   processData: false,
   contentType: false,
   secure: false,
   ...
   $.ajax({
         ...
         success: (data, textStatus, jqXHR) => {
            if((data.responseData).length != 0) { 
              ...
              $('#store_id').val(cart_list.store_id);
              ...
              var available = 1;
               ...
               $('input[name=wallet]').attr('data-amount',cart_list.user_wallet_balance);

               $('input[name=wallet]').attr('data-total',cart_list.payable.toFixed(2));

               $('input[name=wallet]').attr('data-currency',currency);

               $('.promocode_price').html(show_promo);
               if(show_promoid != 0){
                  $(".promocode_price").closest('tr').removeClass('d-none');
               }
               $('#total_amount').val(cart_list.payable);
...
$('input[name=wallet]').on('change',function(e)
{
  if($(this).is(':checked'))
  {
    var total = parseFloat ($(this).attr('data-total')) >parseFloat($(this).attr('data-amount'))? $(this).attr('data-total')-$(this).attr('data-amount') : 0;

    $('.Total').html($(this).attr('data-currency')+(total));

  }
  else
  {
    $('.Total').html($(this).attr('data-currency')+($(this).attr('data-total')));
  }
});
...
$('body').on('click','.placeOrder',function(){
      ...
      var user_address_id = $('#delivery_address_id').val();
       if(order_type=="DELIVERY"){ 
        ...
        if(user_address_id ==''){
          ...
          var msg1 ="Please Select Delivery Address";
          alertMessage("Error",msg1, "danger");
          $('#msg').html(msg1);
         ...
         if(payment_mode=="CARD"){
          alertMessage("Error","Cash Mode Only Allowed", "danger");
          return false;
         }
      }
      ...
      success: (data, textStatus, jqXHR) => { 
         if((data.responseData).length != 0) { 
               ...
               location.replace("{{url('store/order')}}/"+order.id);
         }
         hideLoader();
      },
      error: (jqXHR, textStatus, errorThrown) => {
